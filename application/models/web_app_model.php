...
class Web_App_Model extends CI_Model {
	...
	var $column_order = array('id','nama','value1','value2','quadran'); //set column field database for datatable orderable
	var $column_search = array('id','nama','quadran'); //set column field database for datatable searchable just firstname , lastname , address are searchable
	...
	public function getAllDataJoin($getid)
	{
		// $this->db->order_by('id_post','DESC');
		return $this->db->query();
	}
	public function getAllDatainfo($table)
	{
		$this->db->order_by('kd_info','DESC');
		return $this->db->get($table);
	}
	...
	public function getAllDataLimitedInfo($table,$offset,$limit)
	{
		$this->db->order_by('kd_info','DESC');
		return $this->db->get($table,$limit,$offset);
	}
	public function getAllDataLimitedQuery($offset,$limit)
	{
		return $this->db->query("SELECT * FROM hrm_employee a,hrm_emgroup b, hrm_emsubgroup c,hrm_empersa d,hrm_emorgunit e,hrm_emplans f,hrm_emjob g, hrm_empangkat h WHERE a.persgid=b.persgid and a.perssubgid=c.perssubgid and a.persaid=d.persaid and a.orguid=e.orguid and a.plans=f.plans and a.jobid=g.jobid and a.pangkat=h.pangkat LIMIT ".$offset.",".$limit."");
	}
	...
	public function getLoginData($usr,$psw)
	{
		...
		if(count($q_cek_login->result())>0)
		{
			foreach($q_cek_login->result() as $qck)
			{
				...
				{
					$q_ambil_data = $this->db->query("SELECT * FROM hrm_employee a,hrm_emgroup b, hrm_emsubgroup c,hrm_empersa d,hrm_emorgunit e,hrm_emplans f,hrm_emjob g, hrm_empangkat h WHERE a.pernr='$u' and a.persgid=b.persgid and a.perssubgid=c.perssubgid and a.persaid=d.persaid and a.orguid=e.orguid and a.plans=f.plans and a.jobid=g.jobid and a.pangkat=h.pangkat");
